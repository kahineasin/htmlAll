//最后更新时间20221121
//格式化方式 visualStudioCode 格式化插件 (Prettier - code formatter)
function checkXHRLogin(XHR) {
  if (
    XHR.responseURL !== undefined &&
    (XHR.responseURL.indexOf("/User/Login?ReturnUrl=") > -1 ||
      XHR.responseURL.indexOf("/User/AuthSSO?ReturnUrl=") > -1)
  ) {
...
$pf.isArray = function (field) {
  return field instanceof Array;
};
$pf.isListEmpty = function (list) {
  return list === null || list === undefined || list.length < 1;
};
$pf.isAnyNull = function (list) {
  for (var i = 0; i < list.length; i++) {
    //console.info(list[i]);
    if (list[i] === null || list[i] === undefined) {
      return true;
    }
    if ($pf.isArray(list[i]) && $pf.isListEmpty(list[i])) {
      return true;
    }
...
$pf.isAnyNullAction = function (obj, actionList) {
  if (obj === null || obj === undefined) {
    return true;
  }
  for (var i = 0; i < actionList.length; i++) {
    if (actionList[i](obj) === null || actionList[i](obj) === undefined) {
      return true;
    }
    if (
      $pf.isArray(actionList[i](obj)) &&
      $pf.isListEmpty(actionList[i](obj))
    ) {
      return true;
    }
  }
...
$pf.getProperty = function (obj, list) {
  if (list.length < 1) {
    return obj;
  }
  if (obj === null || obj === undefined) {
  ...
  if (obj.hasOwnProperty(list[0])) {
    return $pf.getProperty(obj[list[0]], list.splice(1, list.length - 1));
  }
  return null;
};
/**
 * 用法$pf.isAnyNullProperty(data,['key1','key2.childKey3']
 */
$pf.isAnyNullProperty = function (obj, pList) {
  if (obj === null || obj === undefined) {
    return true;
  }
  for (var i = 0; i < pList.length; i++) {
    var sList = pList[i].split(".");
    var p = $pf.getProperty(obj, sList);
    //debugger;
    if ($pf.isAnyNull([p])) {
      return true;
    }
...
$pf.getDomType = function (dom) {
  if (dom.nodeName == "INPUT" && dom.nodeType == 1 && dom.type == "checkbox") {
    return $pf.domType.checkbox;
  }
  return $pf.domType.unknown;
};
/*
...
$pf.eachLeaf = function (tree, action) {
  if (null !== tree.Children && undefined !== tree.Children) {
    for (var i = 0; i < tree.Children.length; i++) {
      var a = tree.Children[i];
      if (
        null !== a.Children &&
        undefined !== a.Children &&
        a.Children.length > 0
      ) {
...
/*
 *设置url参数
 * param arr: 支持格式为 obj || [{key,value},...] || [{name,value},...]
 */
$pf.setUrlParams = function (url, arr) {
  ...
  if (arr instanceof Array) {
    for (var i = 0; i < arr.length; i++) {
      //考虑到有数组的情况,必需全部移除再设置
      url = removeSameParam(url, arr[i].name || arr[i].key);
    }
    for (var i = 0; i < arr.length; i++) {
      url = setParam(url, arr[i].name || arr[i].key, arr[i].value);
    }
  } else {
    //object
    for (var i in arr) {
...
$pf.getTableRowData = function (rowDom, columnIdxs) {
  var row = $pf.getJQ(rowDom);
  if (columnIdxs == undefined) {
    columnIdxs = $pf.getTableColumnIdxs(row.parents("table").eq(0));
  }
  var r = {};
  for (var i = 0; i < columnIdxs.length; i++) {
    r[columnIdxs[i]] = row.children().eq(i).text();
  }
  return r;
};
...
$pf.setCheckBoxValue = function (dom, v) {
  if (v == true || v == "true") {
...
window.addEventListener("message", function (event) {
  ...
  for (var dataName in pfIFrameMessageHandlers) {
    if (
      ...
      //  { ["response_"+dataName]: pfIFrameMessageHandlers[dataName](event.data[dataName]) }, //ie不支持这种写法,报错:缺少标识符
      //  "*"
      //);

      var postParam = {};
      postParam["response_" + dataName] = pfIFrameMessageHandlers[dataName](
        event.data[dataName]
      );
      event.source.postMessage(postParam, "*");
    }
  }
});
...
$pf.getMessageFromWindow = function (win, dataName, dataValue, callback) {
  ...
  //win.postMessage({ [dataName]: dataValue }, "*");//ie不支持这种写法,报错:缺少标识符
  var postParam = {};
  postParam[dataName] = dataValue;
  win.postMessage(postParam, "*");
};

$pf.addIFrameMessageListener("getHasWatermark", function (data) {
  return $pf.hasWatermark;
});

$pf.watermark = function (settings) {
  //console.info(parent.$pf.hasWatermark);
  //console.info($pf.hasWatermark);
  //if ($pf.hasWatermark || parent.$pf.hasWatermark) {//以防iframe嵌套的功能出现多层水印(如查询网络图功能)//这种方法跨域报错(以前没发现有问题,现在在管理平台连接时报警)
  //    return;
  //}
  ...
  (TreeMatrix.prototype.setNetByNode = function () {
    ...
    for (var x = 0; x <= me.xMax; x++) {
      for (var y = 1; y <= me.yMax; y++) {
        ...
        //用pfTreeGrid初始化时,只有1层时,会有这种情况--benjamin 20211227
        if (me.node[x + 1] === undefined) me.node[x + 1] = {};
        if (me.node[x + 1][y] === undefined) me.node[x + 1][y] = {};
        if (me.lastChild[x + 1] === undefined) me.lastChild[x + 1] = {};
        if (me.lastChild[x + 1][y] === undefined) me.lastChild[x + 1][y] = {};
...
/*
      提取此方法,是为了便于弹窗上使用
      */
$pf.initFileField = function (dom, opt) {
  ...
  dom.change(function (v) {
    ...
    if (files !== null && files !== undefined && files.length > 0) {
      ...
      if (typeof opt.onFileSelected == "function") {
        opt.onFileSelected();
      }
    }
  });
...
$pf.initCheckBoxField = function (dom, chooseLabel) {
  ...
  ////居中似乎更好看(加了这句后,页面上的反而不居中了)--benjamin 20210824
  //dom.css('verticalAlign', 'middle');
  //chooseLabel.css('verticalAlign', 'middle');
};

$pf.domIsPFSelect = function (element) {
  return element[0].nodeName === "SELECT" && element.hasClass("iselect");
};
$pf.validateErrorPlacement = function (error, element) {
  if (
    $pf.domIsPFSelect(element)
    //element[0].id==='hpos'
  ) {
